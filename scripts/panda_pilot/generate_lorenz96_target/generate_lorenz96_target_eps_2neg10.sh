...
for seed in {0..31}; do
    dense_seed_dir="${project_root}/data/panda_pilot/lorenz96_dense/eps_2neg10/seed_${seed}"
    target_seed_dir="${project_root}/data/panda_pilot/lorenz96_target/eps_2neg10/seed_${seed}"
    ...
    echo "Generating target-sampled Lorenz-96 trajectories with epsilon 2^-10 for seed ${seed}..."
    ...
    echo "Finished target-sampled Lorenz-96 trajectories with epsilon 2^-10 for seed ${seed}."
done
